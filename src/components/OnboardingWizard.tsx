...
const OnboardingWizard: React.FC<OnboardingWizardProps> = ({ isOpen, onClose }) => {
    ...
    const saveZoneConfig = async (zoneIndex: number) => {
        ...
        try {
            ...
            // 4. Save Compensation Config (rain/temp) for TIME/VOLUME modes only
            // FAO-56 has rainfall integrated and doesn't use this compensation path
            const isZoneFao56 = zone.wateringMode === 'fao56_auto' || zone.wateringMode === 'fao56_eco';
            if (!isZoneFao56 && (zone.rainCompEnabled || zone.tempCompEnabled)) {
                await new Promise(r => setTimeout(r, 200));

                try {
                    const compensationConfig: ChannelCompensationConfigData = {
                        channel_id: zone.channel_id,
                        rain: {
                            enabled: zone.rainCompEnabled,
                            sensitivity: zone.rainCompSensitivity / 100, // Convert 0-100% to 0.0-1.0
                            lookback_hours: zone.rainCompLookbackHours,
                            skip_threshold_mm: zone.rainCompSkipThreshold,
                            reduction_factor: zone.rainCompSensitivity / 100, // Use same as sensitivity
                        },
                        temp: {
                            enabled: zone.tempCompEnabled,
                            base_temperature: zone.tempCompBaseTemp,
                            sensitivity: zone.tempCompSensitivity / 100, // Convert 0-100% to 0.0-1.0
                            min_factor: 0.5, // Default: reduce to 50% in cold
                            max_factor: 1.5, // Default: increase to 150% in hot
                        },
                        last_rain_calc_time: 0, // Read-only, ignored on write
                        last_temp_calc_time: 0, // Read-only, ignored on write
                    };

                    await bleService.writeChannelCompensationConfig(compensationConfig);
                    console.log(`[Wizard] Compensation config saved for channel ${zone.channel_id} (rain: ${zone.rainCompEnabled}, temp: ${zone.tempCompEnabled})`);
                } catch (compErr) {
                    console.warn('[Wizard] Failed to save compensation config:', compErr);
                    errors.push('Compensation config');
                }
            }
            ...
            <div className="p-4 space-y-3">
                ...
                {currentZone.enabled && zoneSubStep === 3 && isFao56 && (
                    <>
                        ...
                        {/* Cycle & Soak Auto-Recommendation (based on soil + slope) */}
                        {currentZone.soil && (
                            <CycleSoakAuto
                                ...
                                onChange={(config) => updateCurrentZone({
                                    ...
                                    cycleSoakWateringMin: config.cycleMinutes,
                                    cycleSoakPauseMin: config.soakMinutes
                                })}
                            />
                        )}
                    </>
