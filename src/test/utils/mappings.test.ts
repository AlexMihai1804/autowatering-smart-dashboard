...
describe('mappings.ts', () => {
    describe('PLANT_TYPES', () => {
        ...
        it('should have correct order', () => {
            expect(PLANT_TYPES[0]).toBe('Vegetables');
            expect(PLANT_TYPES[7]).toBe('Custom');
        });
    });
    ...
    describe('getPlantIcon', () => {
        it('should return tomato emoji for Vegetables (index 0)', () => {
            expect(getPlantIcon(0)).toBe('🍅');
        });

        it('should return herb emoji for Herbs (index 1)', () => {
            expect(getPlantIcon(1)).toBe('🌿');
        });

        it('should return flower emoji for Flowers (index 2)', () => {
            expect(getPlantIcon(2)).toBe('🌸');
        });

        it('should return tree emoji for Shrubs (index 3)', () => {
            expect(getPlantIcon(3)).toBe('🌳');
        });

        it('should return conifer emoji for Trees (index 4)', () => {
            expect(getPlantIcon(4)).toBe('🌲');
        });

        it('should return seedling emoji for Lawn (index 5)', () => {
            expect(getPlantIcon(5)).toBe('🌱');
        });

        it('should return cactus emoji for Succulents (index 6)', () => {
            expect(getPlantIcon(6)).toBe('🌵');
        });

        it('should return sparkles emoji for Custom (index 7)', () => {
            expect(getPlantIcon(7)).toBe('✨');
        });

        it('should return sparkles emoji for unknown indices', () => {
            expect(getPlantIcon(8)).toBe('✨');
